...
export default function UpateProduct() {
    ...
    const [formFields,setFormFields] = useState([{name:"name",type:"text",placeholder:"Name of Product",value:"",required:"Y"},
                                                {name:"units",type:"text",placeholder:"Ex: 3-3.2 kg",value:"",required:"N"},
                                                {name:"price",type:"text",placeholder:"$39",value:"",required:"N"},
                                                {name:"offerprice",type:"text",placeholder:"Ex: $36",value:"",required:"N"},
                                                {name:"instock",type:"text",placeholder:"Y/N",value:"",required:"N"},
                                                {name:"active",type:"text",placeholder:"Defaults to Y",value:"",required:"N"}
                                            ]);
    ...
    const funOnChange = (e) =>
{
    ...
    const submitForm = (e) => {
        ...
        const active  = tempFormFields[tempFormFields.findIndex( a => a.name === "active")].value;

    if (tempFormFields.filter ( a => a.errors !== "").length === 0 )
    {
        
       axios.post(`${config.restAPIserver}:${config.restAPIHost}/api/updateproduct`,
            {fileName:"updateProduct",vars:{p_name:name,p_units:units,p_price:price,p_offerprice:offerprice,p_inStock:instock,p_active:active}})
            .then (({data}) => {
                // console.log(data)
                ...
                <SigninContainer className="container">
                    ...
                    <form onSubmit={submitForm}>    
                        <div className="d-flex flex-column" >
                            {formFields.map ((item,i) =>
                                <div className="form-group" key={i}>
                                    ...
                                    <input type={item.type} className="form-control" name={item.name}  placeholder={item.placeholder} value={item.value} onChange={funOnChange}/>
                                    {item.errors &&
                        ...
                        <div className="d-flex justify-content-center mb-3">
                            ...
                            <button className="btn btn-sm forward-btn" type="submit" >UPDATE</button>
                        </div>
                    </form>
                </SigninContainer>
            }
