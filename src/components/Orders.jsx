...
export function OrderDetails(props) {
    const history = useHistory();
    // const [accountInfo] = useContext(accountsContext);
    const query = `select b.NAME,category,QTY,a.PRICE,TOTAL_PRICE TOTAL,to_char(a.ts,'DD-MON-YY HH24:MI') TS from orders a join products b on (a.prodid=b.id) where a.id= `+ props.match.params.id ;
    const [orderDetails,setOrderDetails]= useState([]);
